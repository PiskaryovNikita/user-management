...
<form:form method="post" action="${url}" name="userform"
    ...
    <table align="center" width="300px" class="table">
        ...
        <tr>
            <td align="right">Login <form:hidden path="id"/>
            </td>
            <td><form:input path="login" cssClass="form-control"
                            readonly="true"/></td>
        </tr>
        <tr>
            <td></td>
        ...
        <tr>
            ...
            <td><select name="role" path="role">
                <option value="${user.role}" selected><c:out
                        value="${user.role}"/></option>
                <c:forEach items="${roles}" var="currRole">
                    <c:if test="${currRole ne user.role}">
                        <option value="${currRole}">
                            <c:out value="${currRole}"/></option>
                    </c:if>
                </c:forEach>
            </select></td>
        </tr>
        <tr>
            <td></td>
            <td><p><input id="submitbtn" class="ntSaveFormsSubmit"
                          type="submit" name="Submit" value="Edit"> <a href="/home"
